...
$(OUTPUT_DIR)/www/index.html: libtetris emsdk $(SRC_DIR)/html_template/template.html
	source $(BUILD_DIR)/emsdk/emsdk_env.sh && emcc $(EMCC_FLAGS) $(SRC_DIR)/libtetris/src/libtetris.c $(SRC_DIR)/wasm.c -o $(OUTPUT_DIR)/www/index.html --shell-file $(SRC_DIR)/html_template/template.html -s NO_EXIT_RUNTIME=1 -s "EXPORTED_RUNTIME_METHODS=['ccall']"
